...
class GameScene2 extends Phaser.Scene {
    ...
    preload() {
        this.load.tilemapTiledJSON('map2', 'assets/map1.json');
        this.load.image('backgroundGame2', 'assets/background_game2.png');
        this.load.image('porta', 'assets/Cópia de door.png');
        this.load.image('inimigo', 'assets/Cópia de enemy.png');
        this.load.image('jogador', 'assets/Cópia de player.png');
        this.load.image('chave', 'assets/Cópia de key.png');
    ...
    create() {
        ...
        const tileset = map.addTilesetImage("tileset", "tiles");
        map.createLayer("Ground", tileset, 0, 0);
        this.add.text(100, 100, "", { fontSize: "32px", fill: "#fff" });

        this.player = this.physics.add.sprite(100, 100, 'jogador');
        this.player.setCollideWorldBounds(true);
        this.player.setBounce(0.2);
        
        this.spawnKey();
        this.door = this.physics.add.sprite(500, 200, 'porta');
        this.physics.add.overlap(this.player, this.door, this.enterDoor, null, this);
        
        this.enemies = [];
        
        // Primeiro inimigo - movimento mais rápido
        this.enemies[0] = this.physics.add.sprite(400, 200, 'inimigo');
        this.enemies[0].setVelocity(200, 200);
        this.enemies[0].setBounce(1, 1);
        this.enemies[0].setCollideWorldBounds(true);
        
        // Segundo inimigo - movimento ainda mais rápido
        this.enemies[1] = this.physics.add.sprite(200, 400, 'inimigo');
        this.enemies[1].setVelocity(250, -250);
        this.enemies[1].setBounce(1, 1);
        this.enemies[1].setCollideWorldBounds(true);
        
        // Terceiro inimigo - movimento muito rápido
        this.enemies[2] = this.physics.add.sprite(600, 300, 'inimigo');
        this.enemies[2].setVelocity(-300, 300);
        this.enemies[2].setBounce(1, 1);
        this.enemies[2].setCollideWorldBounds(true);
    ...
    spawnKey() {
        ...
        let x = Phaser.Math.Between(50, 750);
        let y = Phaser.Math.Between(50, 550);
        this.keyItem = this.physics.add.sprite(x, y, 'chave');
        this.physics.add.overlap(this.player, this.keyItem, this.collectKey, null, this);
        this.hasKey = false;
    }
